...
# dbWriteTable(con, "active_summary", active_summary, append = TRUE, row.names = FALSE)
# # dbSendQuery(con, "ALTER TABLE active_summary MODIFY dateComponents DATE;")
# # dbSendQuery(con, "CREATE UNIQUE INDEX idx_dateComponents ON active_summary (dateComponents);")
...
# dbWriteTable(con, "workouts", workouts, append = TRUE, row.names = FALSE)
# # dbSendQuery(con, "ALTER TABLE workouts MODIFY creationDate DATETIME;")
# # dbSendQuery(con, "ALTER TABLE workouts MODIFY startDate DATETIME;")
# # dbSendQuery(con, "ALTER TABLE workouts MODIFY endDate DATETIME;")
# # dbSendQuery(con, "CREATE UNIQUE INDEX idx_creationDate ON workouts (creationDate);")
...
# dbWriteTable(con, "processed", workouts, append = TRUE, row.names = FALSE)
...
#     filter(between(ymd(format(creationDate, "%Y-%m-%d")), start_filter, end_filter))
# dbWriteTable(con, "records", records, append = TRUE, row.names = FALSE)
# # dbSendQuery(con, "ALTER TABLE records MODIFY creationDate DATETIME;")
# # dbSendQuery(con, "ALTER TABLE records MODIFY startDate DATETIME;")
# # dbSendQuery(con, "ALTER TABLE records MODIFY endDate DATETIME;")
# # dbSendQuery(con, "ALTER TABLE records MODIFY type VARCHAR(30);")
# # dbSendQuery(con, "CREATE UNIQUE INDEX idx_startDate_creationDate_endDate_type_value ON records (startDate, creationDate, endDate, type, value);")
